...
class Node {
    ...
    constructor(public node_id: number, public disk_id: number, public pos: vec3) {
        this.node_links = [];
    }
}
...
export class MovementGraph {
            ...
            for (let j = i + 1; j < size; j++) {
                ...
                if (
                    !Collision.circleIntersection(
                        disk_i.position[0],
                        disk_i.position[2],
                        disk_i.radius + this.collision_offset,
                        disk_j.position[0],
                        disk_j.position[2],
                        disk_j.radius + this.collision_offset
                    )
                )
                    continue;
        ...
        while (!this.queue_start.isEmpty()) {
            ...
            for (let link of this.node_list[curr].node_links) {
                ...
                if (g_score < this.search_data[link.dest_node_id].start.given_cost) {
                    ...
                    this.search_data[link.dest_node_id].start.priority =
                        g_score + this.search_data[link.dest_node_id].start.heuristic;
        ...
        while (!this.queue_start.isEmpty() && !this.queue_end.isEmpty()) {
            ...
            for (let link of this.node_list[curr].node_links) {
                ...
                let linked_node_search_data: SearchData = popped_from_start_queue
                    ? this.search_data[link.dest_node_id].start
                    : this.search_data[link.dest_node_id].end;
    ...
    public getPathCost(path: number[]): number {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            ...
            for (let j of this.node_list[path[i]].node_links) {
                if (j.dest_node_id == path[i + 1]) cost += j.weight;
            }
        }
        return cost;
    }
    ...
    private getmmPath(node_start_id: number, node_meeting_id: number, node_end_id: number): number[] {
        ...
        if (curr_node != node_end_id) curr_node = this.search_data[curr_node].end.path_node;
